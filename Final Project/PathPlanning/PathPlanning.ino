/************************************
   Unimate-FinalProject-TopologicalPathFollowing.ino
   Andrew Frush, Val Rumzis, 2.24.26
   ***********************************
   The following program is an implimentation of direction following, where when going forward, the robot follows center (proportional control).
   ...
   topologicalFollow(char command[]): Takes a string of commands and executes them in order. Uses follow center logic to teavel down halways until turning and terminating.
...
char directions[] = {'S','R','L','T'}; //directions for robot to follow
...
/*
  Ensures: Logic for command following. Follows center of hallway until end is detected then maked the relevant turn.
  command[]: String of character commands representing the orders the robot must follow. Actions are done in order. Forward ("S"), left("L"), right("R"), or stop("T").
*/
