...
def isValid(point):
    return (0 <= point.x < 4) and (0 <= point.y < 4) # map is 4 tiles, does not include 4
...
def BFS(mat, start, goal):
    ...
    if not mat[start.x][start.y] or not mat[goal.x][goal.y]: return -1
    ...
    while q:
        ...
        for i in range(4):
            ...
            if isValid(nx, ny, r, c) and mat[nx][ny] and not visited[nx][ny]:
                visited[nx][ny] = True
...
print(BFS(oGrid, Point(0, 3), Point(3, 0)))